...
function AnswerInput({ question, answer, onChange }: { question: ExamQuestion; answer?: SelectedAnswer; onChange: (value: SelectedAnswer) => void }) {
  ...
  if (question.type === "category_sort" || question.type === "table_match") return <div>{question.items?.map(item => <label key={item.id}><ExamText text={item.text} html={item.html} /><select value={values[item.id] ?? ""} onChange={event => { const next = { ...values }; if (event.target.value) next[item.id] = event.target.value; else delete next[item.id]; onChange(next); }}><option value="">Not placed</option>{question.categories?.map(category => <option key={category.id} value={category.id}>{category.title}</option>)}</select></label>)}</div>;
